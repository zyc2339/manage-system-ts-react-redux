...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    if (!userNameVal.trim() || !passwordVal.trim() || !captchaVal.trim()) {
      message.warning("please enter valid information!");
      return;
    }
    //3.if data exist, send login request
    ...
    if (loginAPIRes.code === 200) {
      ...
      //2.save token
      localStorage.setItem("management-system-token", loginAPIRes.token);
      //3.jump to welcome
      navigateTo("/welcome");
      //4.delete uuid
